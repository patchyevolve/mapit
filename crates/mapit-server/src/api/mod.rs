...
async fn annotate_handler(
    ...
    let handle = tokio::task::spawn_blocking(move || {
        ...
        let provider = mapit_ai::provider::create_provider(&global_cfg, &project_cfg)?;
...
async fn simulate_handler(
    ...
    let result =     tokio::task::spawn_blocking(move || {
        ...
        let overview = mapit_core::graph::context::get_project_overview(&store);
        let context = mapit_core::graph::context::build_sim_context(&store, &target, &name, &level)?;
...
async fn ask_handler(
    ...
    let provider = match mapit_ai::provider::create_provider(&global_cfg, &project_cfg) {
        Ok(p) => p,
        Err(e) => {
